...
import getApiCall from "../../../components/baseApiCall";
...
function getData(tokens: any, url: any | null, param: any | null, body: any | null) {
    let apiCall = getApiCall(tokens, "subscriptions", "user", null);
    let sendRes: {
        status: number,
        data: any,
        newToken: any
...
export default async function handler(
    ...
    await getData(tokens, null, null, null).then((response: any) => {
        let planName: any;
        console.log("Response")
        res.setHeader('Set-Cookie', [`tokens=${response.newToken}; Max-Age=3600; Path=/; HttpOnly`]);
        response.data.userSubscriptions.forEach((sub: any) => {
            if (sub.subscriptionStatus === "ACTIVE" && sub.planType === 'SUBSCRIPTION') {
                localStorage.setItem('userPlan', sub.subscriptionPlanName);
