...
const S2s = (props: any) => {
    ...
    const ueConnectUrl = async () => {
        ...
        try {
            const abc = await clientservice('/api/v1/import-account/connectRequest', bodyOfConnectReq, null);
            console.log(abc);
    ...
    bodyOfConnectReq.userDefinedAccountName = userDefinedname;
    bodyOfConnectReq.accountNameInLC = props.parsedAccount.accountNameInLC;
    return (
        (
                    ...
                    <div className={`container ${styles.main_contain} col-12 mt-3`}>
                        <div className="row">
                            ...
                            <div className='row pe-0'>
                                ...
                                <div className={`${styles.content_api} col-lg-7`}>
                                    <form className="form-group" onSubmit={submitForm}>
                                        ...
                                        <p className={styles.content_api_para}><span>CRPTM</span> uses a secure, read-only access for connecting
                                            ...
                                            <a className={styles.content_api_anchor} target="_blank" href={"/integrations/" + props.parsedAccount.accountNameInLC}> Need help? Follow our <span className={styles.text_purple}>step by step guide</span> to connect with {props.parsedAccount.accountDisplayName} </a>
                                        </p>
                                        {isloaders2s === "idle" || isloaders2s === "completed" || isloaders2s === "error" ? <button onClick={() => setSendRequest(true)} type="submit"
                                            className={`btn ${styles.btn_connect}`}>Connect</button> : <img className={styles.loader_connect_s2s} src={imageHost + '/loader2.gif'} alt="loader" />}
                                    </form>
                                </div>
                            </div>
                        </div>
                    </div>
                    {/* <div className="container sub_btn col-lg-6  col-sm-12 mt-3 m-auto d-flex justify-content-center">
